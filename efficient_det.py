...
def get_model_data(client, name):
    meta_data = client.get_model_metadata(model_name = name) 
    pp.pprint(meta_data)
    return meta_data 

client = ovmsclient.make_grpc_client("localhost:9001")

#mdata = get_model_data(client, "yolov7")
mdata = get_model_data(client, "efficientdet-d7")
...
img_raw = cv2.imread("sample_images/image.png", cv2.IMREAD_COLOR)
...
cv2.imwrite("output/result.jpg", img_raw)
